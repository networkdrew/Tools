...
/** Formats the browser can reliably decode via createImageBitmap for editing. */
...
/** The working resolution used for interactive editing; large images are downscaled to this before display, but exports always re-render at full resolution. */
export const MAX_WORKING_DIMENSION = 1600;
...
export function computeWorkingSize(
  ...
  if (longest <= MAX_WORKING_DIMENSION) return { width, height };
  const scale = MAX_WORKING_DIMENSION / longest;
  return {
