...
describe("outputFilename", () => {
  it("replaces the original extension and appends -converted", () => {
    expect(outputFilename("photo.gif", "image/jpeg")).toBe(
      "photo-converted.jpg",
    );
  });

  it("falls back to a generic name when the original has no usable name", () => {
    expect(outputFilename("", "image/webp")).toBe("image-converted.webp");
  });
});
