 ...
 * "real browser" path has to be stubbed to exercise cleaning.
 */
function mockCanvasSupport(outputBlobType = "image/png") {
  vi.stubGlobal(
    "createImageBitmap",
    vi.fn(
      async () =>
...
describe("ImageMetadataRemoverTool", () => {
  ...
  it("cleans a selected image and shows a downloadable result", async () => {
    mockCanvasSupport("image/png");
    ...
    await waitFor(() =>
      expect(
        screen.getByRole("button", { name: "Download cleaned image" }),
      ).toBeInTheDocument(),
    );
  ...
  it("warns that an animated GIF will be flattened and saved as PNG", async () => {
    mockCanvasSupport("image/png");
    const user = userEvent.setup();
    render(<ImageMetadataRemoverTool />);
    ...
    expect(screen.getByText(/saved as PNG instead/i)).toBeInTheDocument();
  });
  ...
  it("clears the selected file and results on Reset", async () => {
    ...
    expect(
      screen.queryByRole("button", { name: "Download cleaned image" }),
